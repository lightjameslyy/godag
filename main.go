	...
	for {
		...
		if flag == true {
			if indx := strings.Index(line, pkgName); indx != -1 {
				srcFullPkg := strings.Trim(line[indx:], "\"")
				skip := false
				for _, skipDir := range strings.Split(skipDirs, ",") {
					if skipDir != "" && strings.HasPrefix(srcFullPkg, pkgName+skipDir) {
						//fmt.Printf("skip pkg: %v\n", srcFullPkg)
						skip = true
						break
					}
				}
				...
				srcPkg := depthParse(srcFullPkg, pkgName)
...
func analyseDir(dir string) error {
	...
	err := filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() {
			for _, skipDir := range strings.Split(skipDirs, ",") {
				if skipDir != "" && strings.HasPrefix(path, pkgPath+skipDir) {
		...
		if !info.IsDir() {
			if strings.Contains(path, ".go") && !strings.HasSuffix(path, "_test.go") && !strings.Contains(path, pkgPath+"/vendor") {
...
func main() {
	...
	flag.StringVar(&skipDirs, "skip_dirs", "", "skip dirs")
